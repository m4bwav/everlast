...
Add the block from `templates/AGENTS.md.snippet` to `AGENTS.md` (or the file the repo's agents actually read; `CLAUDE.md` should only import or point at it, `.github/copilot-instructions.md` likewise). Eight lines; do not add more. In `excluded` mode the block goes into a file the repo does not track: `CLAUDE.local.md` (gitignored by Claude Code) or `~/.claude/<project>.md` imported from there, and `AGENTS.md` stays untouched. Record it: `EVERLAST log <repo> --op update --title "AGENTS.md pointer added"`.
