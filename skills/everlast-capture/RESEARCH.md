...
Topic: how AI coding agents capture session knowledge in modular, on-demand repo documentation. Tier `fast`. Last refresh 2026-09-23 (a correction and the check-before-use pass behind plugin 0.4.0, not a full four-track search; the full pass was 2026-09-22); next due per `evergreen.json`. This is the suite's research hub: `everlast-setup` and `everlast-resume` keep only the findings specific to their own claims and point here for the rest.
...
- Consolidation vocabulary comes from Anthropic's Dreams (the Managed Agents research preview, beta header `dreaming-2026-04-21`, announced 2026-05-19): merge duplicates, replace contradicted entries with the latest value, absolute dates, rebuild the index, and write a new store while leaving the input store untouched for review. Adopted as the prune pass, whose deterministic part is now `everlast.py maintain --apply`. Claude Code's "Auto Dream" never shipped (R-20260923-1), so nothing in Claude Code consolidates memory files on its own.
- Check before use is where the 2026 evidence points for a stored fix: agents check a memory's source about one time in five and act on superseded constraints about three times in four unless forced, and forcing the check adds 61 to 74 points; GitHub Copilot Memory cites code and re-validates it before use (R-20260923-2). So a solution must carry a proof the next session can re-run, a recheck date (`stale_after`), and the other names it will be searched by (`aliases`).
...
- (resolved 2026-09-23) Auto Dream is not in Claude Code: a `/memory` toggle appeared in 2.1.81 (2026-03-20), `/dream` answered "Unknown skill: dream" (anthropics/claude-code#38461, 2026-03-24), and "dream" is absent from the changelog through 2.1.280 and from the memory docs (R-20260923-1). Dreams belongs to Managed Agents memory stores and never touches repo files, so the prune pass has nothing to defer to.
...
- (resolved 2026-09-23) Excluded mode writes a CLAUDE.local.md, which stops Claude Code 2.1.277+ from reading the repository's AGENTS.md unless it imports it; everlast-setup Step 5 now puts `@AGENTS.md` first in CLAUDE.md and in excluded mode's CLAUDE.local.md (everlast-setup C-20260923-1), and the contradiction is cleared.
...
### R-20260923-2 · 2026-09-23 · Subject: check before use changes what a capture must hold
- summary: The plugin's R-20260923-2 (Copilot Memory re-validates cited code before use, merge rate 90% against 83%; forcing a source check added 61 to 74 points in 2608.25553; read-only re-checking lifted a pass rate from 39% to 73% in 2609.11060) and R-20260923-3 (lexical search with aliases holds up at this scale) change the write side: a solution's Verified-by must be a command the next session can re-run, every entry carries `stale_after` (Open Knowledge Format v0.2's name), other names go in `aliases`, `Related:` links carry a label, and a fact in a multi-fact entry can carry its own `(verified YYYY-MM-DD)` stamp.
- track: subject, practice
- sources: https://docs.github.com/en/copilot/concepts/agents/copilot-memory, https://arxiv.org/abs/2608.25553, https://arxiv.org/abs/2609.11060, https://github.com/GoogleCloudPlatform/open-knowledge-format
- magnitude: 0.45
- applied: C-20260923-1 (SKILL.md Step 5: the search before writing, verify on update, note flags, Check-before-use rules; Outcome)

### R-20260923-1 · 2026-09-23 · Subject: correction, Claude Code "Auto Dream" never shipped; Dreams is the Managed Agents research preview
- summary: This file said consolidation vocabulary came from Dreams "and Claude Code's Auto Dream", and asked whether Auto Dream was GA. A `/memory` toggle appeared in Claude Code 2.1.81 (2026-03-20); `/dream` returned "Unknown skill: dream" (anthropics/claude-code#38461, 2026-03-24); "dream" is absent from the changelog through 2.1.280 and from the memory docs. Dreams is the Managed Agents research preview (beta header `dreaming-2026-04-21`, blog 2026-05-19): it reads a store plus 1 to 100 session transcripts and writes a new store, never the input one. The prune pass's vocabulary stands; its source was misattributed.
- track: subject
...
- magnitude: 0.3 (a stated fact was wrong; only an open question depended on it)
- applied: C-20260923-1 (RESEARCH.md Current understanding and Open questions)
