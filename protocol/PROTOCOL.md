...
Link the documents together. A doc set is a small graph, not a pile: every folder a reader might enter has an index file that links each document in it (the generated `INDEX.md` here, a `README.md` elsewhere), every entry links back to its index and to the entries it builds on or supersedes, and a reference to another document is a relative markdown link (`[title](../solutions/2026-09-06-cs0103.md)`), never a bare filename. The reason is twofold: an agent that follows links loads only what the task needs, and a human reading the same files in a graph-aware editor (Obsidian, Foam, Logseq) gets backlinks and a navigable graph for free. Relative markdown links are the one form that works in all of them and on GitHub; wikilinks (`[[...]]`) do not render outside those editors, so do not use them in a shared repository. Rules for the format are in [DOC-TYPES.md](DOC-TYPES.md).
